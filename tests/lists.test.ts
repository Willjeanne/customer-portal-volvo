...
test("lists use fixed read queries and never treat GraphQL errors as empty lists", async (t) => {
  ...
  t.mock.method(globalThis,"fetch",async (url:string,init:RequestInit) => {
    ...
    const body = JSON.parse(String(init.body)); assert.ok(body.query.startsWith("query "));
    return Response.json(fail ? {data:{getLists:[]},errors:[{message:"denied"}]} : {data:{getLists:[]}});
  });
  assert.deepEqual(await getBuyerLists(session),[]);
  fail=true; await assert.rejects(getBuyerLists(session),{code:"LISTS_RESPONSE"});
  await assert.rejects(getBuyerListItems(session,'invalid/../id'));
  context.mode="preview"; await assert.rejects(getBuyerLists(session),{code:"LIVE_REQUIRED"});
});
