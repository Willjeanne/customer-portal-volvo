...
export async function ReplenishmentLists({ session }: { session: PortalSession }) {
  if (session.context.mode === "preview") return <section className="detail-panel"><h2>Replenishment lists</h2><p>Sign in with VTEX to load saved lists.</p><Link className="button secondary" href="/quick-order">Prepare a draft</Link></section>;
  const result = await getBuyerLists(session).then(data => ({data,error:null})).catch((error:unknown) => ({data:null,error}));
  if (!result.data) return <section className="detail-panel" role="alert"><h2>Lists unavailable</h2><p>{result.error instanceof PortalError ? result.error.message : "Lists could not be loaded."}</p><Link href="/lists">Try again</Link></section>;
...
export async function Preparation({session, orderId, listId}:{session:PortalSession;orderId?:string;listId?:string}) {
  ...
  const result = await (async () => {
    ...
    if (parsed.errors.length) throw new PortalError(400,"INVALID_DRAFT",parsed.errors.join(" "));
    return parsed.lines;
  })().then(data => ({data,error:null})).catch((error:unknown) => ({data:null,error}));
  if (!result.data) return <section className="detail-panel" role="alert"><h2>Draft unavailable</h2><p>{result.error instanceof PortalError ? result.error.message : "The selected source could not be loaded."}</p><Link href="/quick-order">Start a blank draft</Link></section>;
  return <QuickOrder key={orderId || listId} initialLines={result.data} source={orderId ? `From order ${orderId}` : "From your VTEX replenishment list"} />;
}
