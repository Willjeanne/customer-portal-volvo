...
async function query<T>(
  ...
  if (session.context.mode !== "vtex" || !session.upstreamCookies)
    throw new PortalError(
      401,
      "LIVE_REQUIRED",
      "Sign in with VTEX to view replenishment lists.",
    );
  let response: Response;
  try {
    response = await fetch(
      "https://www.emeafaststore.com/_v/private/graphql/v1",
      {
        method: "POST",
        headers: {
          ...buyerHeaders(session.upstreamCookies),
          "Content-Type": "application/json",
          "x-vtex-app-id": "vtex.replenishment-service@1.x",
          Origin: "https://www.emeafaststore.com",
        },
    ...
    throw new PortalError(
      502,
      "LISTS_UNAVAILABLE",
      "The lists service could not be reached.",
    );
  }
  ...
  if (
    ...
    throw new PortalError(
      502,
      "LISTS_CONTRACT_UNAVAILABLE",
      "The store rejected the replenishment list query. The active service contract must be checked before lists can be loaded.",
    );
  if (!response.ok)
    throw new PortalError(
      response.status === 403 || response.status === 401 ? 403 : 502,
      "LISTS_UNAVAILABLE",
      `Replenishment lists are unavailable for this session (HTTP ${response.status}).`,
    );
  const parsed = z
    .object({ data: schema.nullish(), errors: z.array(z.unknown()).optional() })
    .safeParse(payload);
  if (!parsed.success || parsed.data.errors?.length || !parsed.data.data)
    throw new PortalError(
      502,
      "LISTS_RESPONSE",
      "The lists service could not return your data. Please try again.",
    );
