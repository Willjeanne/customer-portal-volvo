...
export default async function OrderPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  return <Shell context={session.context}>
    ...
    {!order ? <section className="detail-panel" role="alert"><h2>{result.error instanceof PortalError && result.error.status === 404 ? "Order not found" : "Order unavailable"}</h2><p>{result.error instanceof PortalError ? result.error.message : "This order could not be loaded. Please try again."}</p></section> : <>
      <section className="detail-panel"><h2>{orderStatusLabel(order.status)}</h2><dl><dt>Placed</dt><dd>{new Intl.DateTimeFormat("en-US", { dateStyle: "medium", timeZone: "America/Chicago" }).format(new Date(order.creationDate))}</dd><dt>Total</dt><dd>{orderMoney(order.value, order.storePreferencesData?.currencyCode)}</dd></dl></section>
      <Link className="button secondary" href={`/claims?order=${encodeURIComponent(order.orderId)}`}>Return or report an issue</Link>
